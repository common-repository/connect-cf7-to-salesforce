...
class SalesforceApi {
	...
	public function restoreExistingOAuthConnection( $serializedAuthenticator, $configuration, callable $afterRefresh ) {
		$connector = new Connectors\SalesforceOAuthLoginConnector();

		$authenticator = AccessTokenAuthenticator::unserialize( $serializedAuthenticator );

		// In some reason, configuration is empty, so recreate;
		$connector->setOauthConfiguration( $configuration );
